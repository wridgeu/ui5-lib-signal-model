...
export const config = {
	...
	services: [
		[
			...
			{
				paths: [
					"/test-resources/ui5/model/signal/qunit/testsuite.qunit.html",
				],
			},
		],
	],
 ...
 */
export type ModelPath<T> = "/" | (T extends object ? `/${PathImpl<T, Exclude<keyof T, keyof unknown[]>>}` : never);
...
QUnit.module("SignalRegistry", () => {
	...
	QUnit.test("addComputed creates a computed signal", (assert) => {
		...
		const computed = registry.addComputed("/fullName", ["/firstName", "/lastName"], (first, last) => {
			return `${first} ${last}`;
		});
	...
	QUnit.test("addComputed on existing computed replaces it", (assert) => {
		...
		assert.strictEqual((registry.get("/sum") as Signal.Computed<unknown>).get(), 11, "first computed");

		registry.addComputed("/sum", ["/a"], (a) => (a as number) + 20);
		assert.strictEqual((registry.get("/sum") as Signal.Computed<unknown>).get(), 21, "replaced computed");
...
QUnit.module("SignalModel", () => {
	...
	QUnit.test("setData fires all signals on replace", (assert) => {
		...
		nameBinding.attachChange(() => { nameChanged = true; });
		ageBinding.attachChange(() => { ageChanged = true; });
	...
	QUnit.test("parent path signals fire on leaf write", (assert) => {
		...
		customerBinding.attachChange(() => { customerChanged = true; });
...
export default class SignalModel extends ClientModel {
	...
	setData(oData: Record<string, unknown>, bMerge?: boolean): void {
		if (bMerge) {
			this.oData = deepExtend(
				Array.isArray(this.oData) ? [] : {},
				this.oData,
				oData,
	...
	setProperty(sPath: string, oValue: unknown, oContext?: Context, bAsyncUpdate?: boolean): boolean {
		...
		if (oObject) {
			...
			if (typeof oValue === "object" && oValue !== null) {
				this.registry.invalidateChildren(sResolvedPath, (path: string) =>
					this._getObject(path),
				);
			}
	...
	override bindProperty(sPath: string, oContext?: Context, mParameters?: object): SignalPropertyBinding {
...
QUnit.module("SignalListBinding", () => {
	...
	QUnit.test("binding to unrelated path does not fire", (assert) => {
		...
		binding.attachChange(() => { changed = true; });
...
export default class SignalListBinding extends ClientListBinding {
	...
	subscribe(): void {
		...
		const signal = this.oModel._getOrCreateSignal(resolvedPath, this.oModel._getObject(resolvedPath));
...
export default class StrictMode extends Controller {
	...
	override onInit(): void {
		this.strictModel = new SignalModel(
			{ name: "Alice", age: 28 },
			{ strict: true },
		);
...
// Computed signals
model.createComputed("/fullName", ["/firstName", "/lastName"],
